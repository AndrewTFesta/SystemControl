...
from SystemControl.OBciPython.EHub.GanglionMessages import STOP_SCAN, connect_board
    ...
    if res_code == 200:
        if action_type == 'start':
            ...
            device.msg_log.log_message(
                message_ident=SystemLogIdent.LOG,
                message_level=SystemLogLevel.ERROR,
                message_string=f'Unrecognized scan action type: {action_type}'
            )
    elif res_code == 201:
        device.scan = True
...
def handle_message_connect(device, data_dict):
    ...
    if res_code == 200:
        ...
        device.msg_log.log_message(
            ...
            message_string=f'Board connected: {device.board_name}'
        )
    elif res_code == 402:
        ...
        device.msg_log.log_message(
            message_ident=SystemLogIdent.LOG,
            message_level=SystemLogLevel.ERROR,
            message_string='Unrecognized board connect result code: {}'.format(res_code)
        )
    return
...
def handle_message_disconnect(device, data_dict):
    ...
    if res_code == 200:
        ...
        device.msg_log.log_message(
            message_ident=SystemLogIdent.LOG,
            message_level=SystemLogLevel.ERROR,
            message_string='Unrecognized board disconnect result code: {}'.format(res_code)
        )
    return
...
def handle_message_command(device, data_dict):
    ...
    if res_code == 200:
        ...
        device.msg_log.log_message(
            message_ident=SystemLogIdent.LOG,
            message_level=SystemLogLevel.ERROR,
            message_string='Unrecognized command result code: {}'.format(res_code)
        )
    return
